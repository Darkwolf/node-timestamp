...
const toFields = value => {
  ...
  if (offset !== undefined) {
    fields.offset = toDateAmount(offset, 'offset')
  }
  return fields
}
...
const _parseTimestampToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]):([0-5]\d):([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?(Z|([+-])([01]\d|2[0-3]):([0-5]\d))$/)
  if (!match) {
    return null
  }
  let [
  ...
  if (timezone === ZERO_TIMEZONE_CHAR) {
    fields.offset = 0
  } else {
    const isNegativeOffset = offsetSign === NEGATIVE_CHAR
    hourOffset = Number(hourOffset)
    minuteOffset = Number(minuteOffset)
    const offset = hourOffset * SECONDS_PER_HOUR + minuteOffset * SECONDS_PER_MINUTE
    fields.offset = isNegativeOffset && offset ? -offset : offset
  }
  return fields
}
...
const _parseOffsetToParts = string => {
  if (string === ZERO_TIMEZONE_CHAR) {
    return {
      hours: 0,
      minutes: 0
    }
  }
  const match = StringPrototypeMatch(string, /^([+-])([01]\d|2[0-3])(?::?([0-5]\d))?$/)
  if (!match) {
    return null
  }
  let [input, sign, hours, minutes] = match
...
const _parseTimeOffsetToFields = string => {
  const match = StringPrototypeMatch(string, /^([01]\d|2[0-3]):?([0-5]\d)(?::?([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?(?:\s?(Z|([+-])([01]\d|2[0-3])(?::?([0-5]\d))?))?$/)
  if (!match) {
    return null
  }
  let [
  ...
  if (timezone !== undefined) {
    if (timezone === ZERO_TIMEZONE_CHAR) {
      ...
      const isNegativeOffset = offsetSign === NEGATIVE_CHAR
      hourOffset = Number(hourOffset)
      let offset = hourOffset * SECONDS_PER_HOUR
      if (minuteOffset !== undefined) {
        minuteOffset = Number(minuteOffset)
        offset += minuteOffset * SECONDS_PER_MINUTE
      }
      fields.offset = isNegativeOffset && offset ? -offset : offset
    }
  }
...
const parseTimeOffsetToFields = string => {
  string = String(string)
  return _parseTimeOffsetToFields(string)
}

const _parseDateTimeToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-?(0[1-9]|1[0-2])-?(0[1-9]|[12]\d|3[01])[T\s]([01]\d|2[0-3]):?([0-5]\d)(?::?([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?(?:\s?(Z|([+-])([01]\d|2[0-3])(?::?([0-5]\d))?))?$/)
  if (!match) {
    return null
  }
  let [
...
class Timestamp {
  ...
  [toStringSymbol](options) {
    ...
    return `${result}${ZERO_TIMEZONE_CHAR}`
  }
...
ObjectDefineProperties(Timestamp, {
  ...
  parseTimeOffsetToFields: {
    value: parseTimeOffsetToFields
  },
