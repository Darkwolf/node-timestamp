...
import Duration, {
  ...
  fromParts as DurationFromParts
...
const DurationPrototypeToTimeString = uncurryThis(Duration.prototype.toTimeString)
...
const _parseDateTimeToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])\s+([01]?\d|2[0-3]):([0-5]\d)(?::([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?$/)
  if (!match) {
    return null
  }
  let [
...
const toSlots = input => {
  ...
  if (DurationIsDuration(input)) {
    return DurationPrototypeToSlots(input)
  }
  if (InstancesIsDate(input)) {
    ...
    return DurationConvertToSlots(timestamp, 'millisecond')
  }
  return TypesIsPlainObject(input) ? fieldsToSlots(input) : timestampToSlots(input)
}
...
class Timestamp {
  constructor(...args) {
    ...
    if (!length) {
      const timestamp = DateNow()
      const slots = DurationConvertToSlots(timestamp, 'millisecond')
  ...
  [toStringSymbol](options) {
    ...
    let result = `${StringPrototypePadStart(fields.year, 4, 0)}-${StringPrototypePadStart(fields.month, 2, 0)}-${StringPrototypePadStart(fields.day, 2, 0)}`
    const duration = DurationFromParts({
      hours: fields.hour,
    ...
    const timeString = DurationPrototypeToTimeString(duration, {
      smallestUnit,
      trailingZerosDisplay
    })
    return `${result}T${timeString}Z`
  }
  ...
  toDateString(options) {
    ...
    return `${StringPrototypePadStart(fields.year, 4, 0)}-${StringPrototypePadStart(fields.month, 2, 0)}-${StringPrototypePadStart(fields.day, 2, 0)}`
  }
  ...
  toTimeString(options) {
    ...
    return DurationPrototypeToTimeString(duration, {
      smallestUnit,
      leadingZeroDisplay,
      secondDisplay,
      trailingZerosDisplay
