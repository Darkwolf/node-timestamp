...
import Duration, {
  ...
  MONTHS_PER_YEAR,
  UNIT,
  isDuration as DurationIsDuration,
  getUnitIndex as DurationGetUnitIndex,
  wrapSlots,
  convert as DurationConvert,
  slotsToDuration as DurationSlotsToDuration,
  slotsToParts as DurationSlotsToParts,
  convertToSlots as DurationConvertToSlots,
  partsToDuration as DurationPartsToDuration,
  partsToSlots as DurationPartsToSlots,
  dateToSlots as DurationDateToSlots,
  toSlots as DurationToSlots
} from '@darkwolf/duration'
...
const toTimeStringSymbol = Symbol('toTimeString')
const toDateTimeStringSymbol = Symbol('toDateTimeString')
...
const MIN_OFFSET_HOURS = -MAX_OFFSET_HOURS

const MAX_OFFSET = MAX_OFFSET_HOURS * SECONDS_PER_HOUR
...
const toFields = value => {
  ...
  if (offsetHours !== undefined) {
    fields.offsetHours = toAmount(offsetHours, 'offsetHours')
  }
  if (offsetMinutes !== undefined) {
    fields.offsetMinutes = toAmount(offsetMinutes, 'offsetMinutes')
  }
  if (offsetSeconds !== undefined) {
    fields.offsetSeconds = toAmount(offsetSeconds, 'offsetSeconds')
  }
  return fields
}
...
const _parseTimestampToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])T([01]\d|2[0-3]):([0-5]\d):([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?(Z|([+-])(0\d|1[0-8])(?::((?<!18:?)[0-5]\d|00)))$/)
  if (!match) {
    return null
  }
  let [
  ...
  if (offset === UTC_OFFSET) {
    fields.offsetHours = 0
    fields.offsetMinutes = 0
  } else {
    const isNegativeOffset = offsetSign === NEGATIVE_CHAR
    offsetHours = Number(offsetHours)
    offsetMinutes = Number(offsetMinutes)
    fields.offsetHours = isNegativeOffset && offsetHours ? -offsetHours : offsetHours
    fields.offsetMinutes = isNegativeOffset && offsetMinutes ? -offsetMinutes : offsetMinutes
  }
  return fields
}
...
const _parseOffsetToParts = string => {
  if (string === UTC_OFFSET) {
    return {
      hours: 0,
      minutes: 0
    }
  }
  const match = StringPrototypeMatch(string, /^([+-])(0\d|1[0-8])(?::?((?<!18:?)[0-5]\d|00)(?::?((?<!18:?00:?)[0-5]\d|00))?)?$/)
  if (!match) {
    return null
  }
...
const parseOffsetToParts = string => {
  string = String(string)
  return _parseOffsetToParts(string)
}

const _parseOffset = (string, unit, options) => {
  if (string === UTC_OFFSET) {
    return 0
  }
  const parts = _parseOffsetToParts(string)
  if (!parts) {
    return NaN
  }
  return DurationPartsToDuration(parts, unit, options)
}
const parseOffset = (string, unit, options) => {
  string = String(string)
  return _parseOffset(string, unit, options)
}

const _parseOffsetTimeToFields = string => {
  const match = StringPrototypeMatch(string, /^([01]\d|2[0-3]):?([0-5]\d)(?::?([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?(?:\s?(Z|([+-])(0\d|1[0-8])(?::?((?<!18:?)[0-5]\d|00)(?::?((?<!18:?00:?)[0-5]\d|00))?)?))$/)
  if (!match) {
    return null
  }
  let [
  ...
  if (offset === UTC_OFFSET) {
    ...
    if (offsetSeconds !== undefined) {
      ...
      fields.offsetSeconds = isNegativeOffset && offsetSeconds ? -offsetSeconds : offsetSeconds
    }
  }
  return fields
}
const parseOffsetTimeToFields = string => {
  string = String(string)
  return _parseOffsetTimeToFields(string)
}

const _parseDateTimeToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-?(0[1-9]|1[0-2])-?(0[1-9]|[12]\d|3[01])[T\s]([01]\d|2[0-3]):?([0-5]\d)(?::?([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?$/)
  if (!match) {
    return null
  }
  let [
...
const _parseOffsetDateTimeToFields = string => {
  const match = StringPrototypeMatch(string, /^([+-]?\d{4,9})-?(0[1-9]|1[0-2])-?(0[1-9]|[12]\d|3[01])[T\s]([01]\d|2[0-3]):?([0-5]\d)(?::?([0-5]\d)(?:\.(\d{1,3})(\d{1,3})?(\d{1,3})?)?)?(?:\s?(Z|([+-])(0\d|1[0-8])(?::?((?<!18:?)[0-5]\d|00)(?::?((?<!18:?00:?)[0-5]\d|00))?)?))$/)
  if (!match) {
    return null
  }
  let [
...
const parseOffsetDateTimeToFields = string => {
  string = String(string)
  return _parseOffsetDateTimeToFields(string)
}

const _parseOffsetDateTime = (string, unit, options) => {
  const fields = _parseOffsetDateTimeToFields(string)
  if (!fields) {
    return NaN
  }
  const slots = _fieldsToSlots(fields)
...
const parseOffsetDateTime = (string, unit, options) => {
  string = String(string)
  return _parseOffsetDateTime(string, unit, options)
}
...
const _parseToFields = string => {
  ...
  if (hour !== undefined) {
    fields.hour = Number(hour)
  }
  if (minute !== undefined) {
    fields.minute = Number(minute)
  }
  if (second !== undefined) {
    fields.second = Number(second)
  }
...
const _parse = (string, unit, options) => {
  const fields = _parseToFields(string)
  if (!fields) {
    return NaN
  }
  const slots = _fieldsToSlots(fields)
...
const timestampToSlots = timestamp => {
  ...
  return DurationConvertToSlots(timestamp, 'second')
}
...
const toOffset = (input, unit, options) => {
  if (unit === undefined) {
    unit = UNIT
  }
  if (PrimitivesIsString(input)) {
    const offset = _parseOffset(input, unit, options)
    if (NumberIsNaN(offset)) {
      throw new RangeError('The offset must be a finite number')
    }
    return offset
  }
  let offset = null
  if (DurationIsDuration(input)) {
    offset = DurationPrototypeConvertTo(input, 'second')
  } else if (TypesIsPlainObject(input)) {
    offset = DurationPartsToDuration(input)
  } else {
    offset = TypesToNumber(input)
    if (!NumberIsFinite(offset)) {
      throw new RangeError('The offset must be a finite number')
    }
  }
  offset = MathTrunc(offset) || 0
  if (offset < MIN_OFFSET) {
    throw new RangeError('The offset must be less than or equal to -18:00')
  }
  if (offset > MAX_OFFSET) {
    throw new RangeError('The offset must be greater than or equal to +18:00')
  }
  return DurationConvert(offset, 'second', unit, options)
}
...
class Timestamp {
  ...
  [toStringSymbol](options) {
    ...
    let result = `${year < 0 ? `${NEGATIVE_CHAR}${StringPrototypePadStart(-year, 4, 0)}` : `${StringPrototypePadStart(year, 4, 0)}`}-${
      ...
      StringPrototypePadStart(second, 2, 0)}`
    if (millisecond !== undefined) {
  ...
  toTimeString(options) {
    return this[toTimeStringSymbol](options)
  }

  toOffsetTimeString(options) {
    const result = this[toTimeStringSymbol](options)
    return `${result}${UTC_OFFSET}`
  }

  [toDateTimeStringSymbol](options) {
    if (options === undefined) {
      options = {}
    } else if (!TypesIsPlainObject(options)) {
      throw new TypeError('The options must be a plain object')
  ...
  toDateTimeString(options) {
    return this[toDateTimeStringSymbol](options)
  }

  toOffsetDateTimeString(options) {
    const result = this[toDateTimeStringSymbol](options)
    return `${result}${UTC_OFFSET}`
  }
...
ObjectDefineProperties(Timestamp, {
  ...
  MAX_OFFSET_HOURS: {
    value: MAX_OFFSET_HOURS
  },
  MIN_OFFSET_HOURS: {
    value: MIN_OFFSET_HOURS
  },
  MAX_OFFSET: {
    value: MAX_OFFSET
  },
  ...
  parseOffsetToParts: {
    value: parseOffsetToParts
  },
  parseOffset: {
    value: parseOffset
  },
  parseOffsetTimeToFields: {
    value: parseOffsetTimeToFields
  },
  ...
  parseOffsetDateTimeToFields: {
    value: parseOffsetDateTimeToFields
  },
  parseOffsetDateTime: {
    value: parseOffsetDateTime
  },
  parseToFields: {
    value: parseToFields
  },
